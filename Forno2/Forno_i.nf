...
THEORY ListVariablesX IS
  ...
  Abstract_List_Variables(Implementation(Forno_i))==(qtd_receita,qtd_ing_estoque,qtd_alimento,r_in_index,existe,r_trava,r_estoque,r_ingredientes,r_receita,r_alimento,r_tempo,r_potencia,qtdIng,tempoDeUso,index_in,pot,trav,cron,temporizacaoR,potenciaR,estoqueR,receita_in_qntR,receita_al_inR);
...
THEORY ListVisibleVariablesX IS
  ...
  List_VisibleVariables(Implementation(Forno_i))==(qtd_ing_estoque,qtd_alimento,qtd_receita,r_in_index,r_trava,r_estoque,r_ingredientes,r_receita,r_alimento,r_tempo,r_potencia);
  Internal_List_VisibleVariables(Implementation(Forno_i))==(qtd_ing_estoque,qtd_alimento,qtd_receita,r_in_index,r_trava,r_estoque,r_ingredientes,r_receita,r_alimento,r_tempo,r_potencia)
END
&
THEORY ListInvariantX IS
  Gluing_Seen_List_Invariant(Implementation(Forno_i))==(btrue);
  Expanded_List_Invariant(Implementation(Forno_i))==(btrue);
  Abstract_List_Invariant(Implementation(Forno_i))==(qtd_alimento: NAT & qtd_ing_estoque: NAT & qtd_receita: NAT & existe: INGREDIENTES --> NAT & r_in_index: INGREDIENTES --> NAT & r_trava: NAT & r_potencia: 0..15 --> NAT1 & r_potencia[0..15] = ran(potenciaR) & r_tempo: 0..15 --> NAT1 & r_tempo[0..15] = ran(temporizacaoR) & r_estoque: 0..15 --> NAT & r_estoque[0..15] = ran(estoqueR) & r_alimento: 0..15 --> NAT & r_ingredientes: 0..15 --> INGREDIENTES & r_receita: ALIMENTO --> NAT & receita_al_inR: ALIMENTO <-> POW(INGREDIENTES) & receita_in_qntR: POW(INGREDIENTES) <-> NAT1 & estoqueR: INGREDIENTES +-> 0..10 & potenciaR: ALIMENTO +-> NAT & temporizacaoR: ALIMENTO +-> NAT & cron: NAT & pot: NAT & trav: NAT & index_in: 0..card(dom(estoqueR)) +-> INGREDIENTES & tempoDeUso: NAT & qtdIng: 0..card(dom(estoqueR)));
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(Forno_i))==((0: dom(r_potencia) & 0: INT | r_potencia:=r_potencia<+{0|->0});(0: dom(r_tempo) & 0: INT | r_tempo:=r_tempo<+{0|->0});(0: dom(r_estoque) & 0: INT | r_estoque:=r_estoque<+{0|->0});(0: dom(r_ingredientes) & 0: INT | r_ingredientes:=r_ingredientes<+{0|->0});(0: dom(r_receita) & 0: INT | r_receita:=r_receita<+{0|->0});(0: dom(r_alimento) & 0: INT | r_alimento:=r_alimento<+{0|->0});(0: INT | r_trava:=0);(0: dom(r_in_index) & 1: INT | r_in_index:=r_in_index<+{0|->1});(0: INT | qtd_receita:=0);(0: INT | qtd_alimento:=0);(0: INT | qtd_ing_estoque:=0));
  Context_List_Initialisation(Implementation(Forno_i))==(skip);
  List_Initialisation(Implementation(Forno_i))==(r_potencia(0):=0;r_tempo(0):=0;r_estoque(0):=0;r_ingredientes(0):=0;r_receita(0):=0;r_alimento(0):=0;r_trava:=0;r_in_index(0):=1;qtd_receita:=0;qtd_alimento:=0;qtd_ing_estoque:=0)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Implementation(Forno_i))==(numIngredientesNoEstoque,qntIngredienteNoEstoque,travarForno,destravarForno,inserirNovoIngredienteNoEstoque,incrementarIngredienteDoEstoque,decrementarIngredienteDoEstoque,atualizarReceitaAdicinandoIngrediente,cadastrarReceita,fazerAlimento);
  List_Operations(Implementation(Forno_i))==(numIngredientesNoEstoque,qntIngredienteNoEstoque,travarForno,destravarForno,inserirNovoIngredienteNoEstoque,incrementarIngredienteDoEstoque,decrementarIngredienteDoEstoque,atualizarReceitaAdicinandoIngrediente,cadastrarReceita,fazerAlimento)
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Implementation(Forno_i),qntIngredienteNoEstoque)==(ii: INGREDIENTES & ii: dom(estoqueR));
  Own_Precondition(Implementation(Forno_i),travarForno)==(btrue);
  List_Precondition(Implementation(Forno_i),travarForno)==(r_trava = 1 & trav = 1);
  Own_Precondition(Implementation(Forno_i),destravarForno)==(btrue);
  List_Precondition(Implementation(Forno_i),destravarForno)==(r_trava = 0 & trav = 0);
  Own_Precondition(Implementation(Forno_i),inserirNovoIngredienteNoEstoque)==(btrue);
  List_Precondition(Implementation(Forno_i),inserirNovoIngredienteNoEstoque)==(ii: INGREDIENTES & qq: NAT & qq<10 & qq>0 & ii/:dom(estoqueR));
  Own_Precondition(Implementation(Forno_i),incrementarIngredienteDoEstoque)==(btrue);
  List_Precondition(Implementation(Forno_i),incrementarIngredienteDoEstoque)==(ii: INGREDIENTES & qq: NAT & qq<10 & qq>0 & ii: dom(estoqueR) & estoqueR(ii)<10);
  Own_Precondition(Implementation(Forno_i),decrementarIngredienteDoEstoque)==(btrue);
  List_Precondition(Implementation(Forno_i),decrementarIngredienteDoEstoque)==(ii: INGREDIENTES & qq: NAT & qq<10 & qq>0 & ii: dom(estoqueR) & estoqueR(ii)<10);
  Own_Precondition(Implementation(Forno_i),atualizarReceitaAdicinandoIngrediente)==(btrue);
  List_Precondition(Implementation(Forno_i),atualizarReceitaAdicinandoIngrediente)==(ii: INGREDIENTES & aa: ALIMENTO & aa: dom(temporizacaoR) & aa: dom(potenciaR));
  Own_Precondition(Implementation(Forno_i),cadastrarReceita)==(btrue);
  List_Precondition(Implementation(Forno_i),cadastrarReceita)==(ii: INGREDIENTES & aa: ALIMENTO & tt: NAT1 & pp: NAT1 & aa/:dom(temporizacaoR) & aa/:dom(potenciaR));
  Own_Precondition(Implementation(Forno_i),fazerAlimento)==(btrue);
  List_Precondition(Implementation(Forno_i),fazerAlimento)==(r_trava = 1 & aa: ALIMENTO & aa: dom(receita_al_inR) & trav = 1 & tempoDeUso<MAXINT)
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(Forno_i),fazerAlimento)==(r_trava = 1 & aa: ALIMENTO & aa: dom(receita_al_inR) & trav = 1 & tempoDeUso<MAXINT | @count.(count:=0;WHILE count<=qtd_alimento DO count = aa ==> (count: dom(r_ingredientes) & r_ingredientes(count): dom(r_in_index) & r_in_index(r_ingredientes(count)): dom(r_estoque) & r_estoque(r_in_index(r_ingredientes(count)))-1: INT & r_estoque(r_in_index(r_ingredientes(count))): INT & 1: INT | r_estoque:=r_estoque<+{r_in_index(r_ingredientes(count))|->r_estoque(r_in_index(r_ingredientes(count)))-1}) [] not(count = aa) ==> skip;(count+1: INT & count: INT & 1: INT | count:=count+1) INVARIANT count: NAT VARIANT qtd_alimento-count END));
  Expanded_List_Substitution(Implementation(Forno_i),cadastrarReceita)==(ii: INGREDIENTES & aa: ALIMENTO & tt: NAT1 & pp: NAT1 & aa/:dom(temporizacaoR) & aa/:dom(potenciaR) | (aa: dom(r_receita) & qtd_receita+1: INT & qtd_receita: INT & 1: INT | r_receita:=r_receita<+{aa|->qtd_receita+1});(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT & qtd_alimento+1: dom(r_alimento) & r_receita(aa): INT & aa: dom(r_receita) | r_alimento:=r_alimento<+{qtd_alimento+1|->r_receita(aa)});(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT & qtd_alimento+1: dom(r_ingredientes) & ii: INT | r_ingredientes:=r_ingredientes<+{qtd_alimento+1|->ii});(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT & qtd_alimento+1: dom(r_tempo) & tt: INT | r_tempo:=r_tempo<+{qtd_alimento+1|->tt});(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT & qtd_alimento+1: dom(r_potencia) & pp: INT | r_potencia:=r_potencia<+{qtd_alimento+1|->pp});(qtd_receita+1: INT & qtd_receita: INT & 1: INT | qtd_receita:=qtd_receita+1);(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT | qtd_alimento:=qtd_alimento+1));
  Expanded_List_Substitution(Implementation(Forno_i),atualizarReceitaAdicinandoIngrediente)==(ii: INGREDIENTES & aa: ALIMENTO & aa: dom(temporizacaoR) & aa: dom(potenciaR) | (qtd_alimento+1: INT & qtd_alimento: INT & 1: INT & qtd_alimento+1: dom(r_alimento) & r_receita(aa): INT & aa: dom(r_receita) | r_alimento:=r_alimento<+{qtd_alimento+1|->r_receita(aa)});(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT & qtd_alimento+1: dom(r_ingredientes) & ii: INT | r_ingredientes:=r_ingredientes<+{qtd_alimento+1|->ii});(qtd_alimento+1: INT & qtd_alimento: INT & 1: INT | qtd_alimento:=qtd_alimento+1));
  Expanded_List_Substitution(Implementation(Forno_i),decrementarIngredienteDoEstoque)==(ii: INGREDIENTES & qq: NAT & qq<10 & qq>0 & ii: dom(estoqueR) & estoqueR(ii)<10 | @xx.((ii: dom(r_in_index) & r_in_index(ii): dom(r_estoque) & r_estoque(r_in_index(ii))-qq: INT & r_estoque(r_in_index(ii)): INT & qq: INT | xx:=r_estoque(r_in_index(ii))-qq);(xx>0 ==> (ii: dom(r_in_index) & r_in_index(ii): dom(r_estoque) & r_estoque(r_in_index(ii))+qq: INT & r_estoque(r_in_index(ii)): INT & qq: INT | r_estoque:=r_estoque<+{r_in_index(ii)|->r_estoque(r_in_index(ii))+qq}) [] not(xx>0) ==> (ii: dom(r_in_index) & r_in_index(ii): dom(r_estoque) & 0: INT | r_estoque:=r_estoque<+{r_in_index(ii)|->0}))));
  Expanded_List_Substitution(Implementation(Forno_i),incrementarIngredienteDoEstoque)==(ii: INGREDIENTES & qq: NAT & qq<10 & qq>0 & ii: dom(estoqueR) & estoqueR(ii)<10 | @xx.((ii: dom(r_in_index) & r_in_index(ii): dom(r_estoque) & r_estoque(r_in_index(ii))+qq: INT & r_estoque(r_in_index(ii)): INT & qq: INT | xx:=r_estoque(r_in_index(ii))+qq);(xx<10 ==> (ii: dom(r_in_index) & r_in_index(ii): dom(r_estoque) & r_estoque(r_in_index(ii))+qq: INT & r_estoque(r_in_index(ii)): INT & qq: INT | r_estoque:=r_estoque<+{r_in_index(ii)|->r_estoque(r_in_index(ii))+qq}) [] not(xx<10) ==> (ii: dom(r_in_index) & r_in_index(ii): dom(r_estoque) & 10: INT | r_estoque:=r_estoque<+{r_in_index(ii)|->10}))));
  Expanded_List_Substitution(Implementation(Forno_i),inserirNovoIngredienteNoEstoque)==(ii: INGREDIENTES & qq: NAT & qq<10 & qq>0 & ii/:dom(estoqueR) | @(ingrediente,count,achou).(ingrediente:=0;count:=0;achou:=0;WHILE count<=qtd_ing_estoque DO count = ii ==> (1: INT | achou:=1) [] not(count = ii) ==> skip;(count+1: INT & count: INT & 1: INT | count:=count+1) INVARIANT ingrediente: INGREDIENTES & count: NAT VARIANT qtd_ing_estoque-count END;(achou = 0 ==> ((qtd_ing_estoque+1: INT & qtd_ing_estoque: INT & 1: INT & qtd_ing_estoque+1: dom(r_estoque) & qq: INT | r_estoque:=r_estoque<+{qtd_ing_estoque+1|->qq});(ii: dom(r_in_index) & qtd_ing_estoque+1: INT & qtd_ing_estoque: INT & 1: INT | r_in_index:=r_in_index<+{ii|->qtd_ing_estoque+1});(qtd_ing_estoque+1: INT & qtd_ing_estoque: INT & 1: INT | qtd_ing_estoque:=qtd_ing_estoque+1)) [] not(achou = 0) ==> skip)));
  Expanded_List_Substitution(Implementation(Forno_i),destravarForno)==(r_trava = 0 & trav = 0 & 1: INT | r_trava:=1);
  Expanded_List_Substitution(Implementation(Forno_i),travarForno)==(r_trava = 1 & trav = 1 & 0: INT | r_trava:=0);
  Expanded_List_Substitution(Implementation(Forno_i),qntIngredienteNoEstoque)==(ii: INGREDIENTES & ii: dom(estoqueR) | @(ingrediente,count).(ingrediente:=0;count:=0;WHILE ingrediente/=ii DO (r_ingredientes(r_in_index(ii)): INT & ii: dom(r_in_index) & r_in_index(ii): dom(r_ingredientes) | ingrediente:=r_ingredientes(r_in_index(ii)));(count+1: INT & count: INT & 1: INT | count:=count+1) INVARIANT ingrediente: INGREDIENTES & count: NAT VARIANT qtd_ing_estoque-count END;(r_estoque(count): INT & count: dom(r_estoque) | qq:=r_estoque(count))));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(Forno_i)) == (? | ? | ? | ? | numIngredientesNoEstoque,qntIngredienteNoEstoque,travarForno,destravarForno,inserirNovoIngredienteNoEstoque,incrementarIngredienteDoEstoque,decrementarIngredienteDoEstoque,atualizarReceitaAdicinandoIngrediente,cadastrarReceita,fazerAlimento | ? | ? | ? | Forno_i);
  List_Of_HiddenCst_Ids(Implementation(Forno_i)) == (? | ?);
  List_Of_VisibleCst_Ids(Implementation(Forno_i)) == (?);
  List_Of_VisibleVar_Ids(Implementation(Forno_i)) == (qtd_ing_estoque,qtd_alimento,qtd_receita,r_in_index,r_trava,r_estoque,r_ingredientes,r_receita,r_alimento,r_tempo,r_potencia | ?);
...
THEORY TypingPredicateX IS
  TypingPredicate(Implementation(Forno_i))==(r_potencia: POW(INTEGER*INTEGER) & r_tempo: POW(INTEGER*INTEGER) & r_alimento: POW(INTEGER*INTEGER) & r_receita: POW(INTEGER*INTEGER) & r_ingredientes: POW(INTEGER*INTEGER) & r_estoque: POW(INTEGER*INTEGER) & r_trava: INTEGER & r_in_index: POW(INTEGER*INTEGER) & qtd_receita: INTEGER & qtd_alimento: INTEGER & qtd_ing_estoque: INTEGER)
END
&
